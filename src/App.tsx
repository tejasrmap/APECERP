...
export default function App() {
  return (
    <div className="min-h-screen w-full bg-slate-950 flex flex-col relative overflow-hidden font-sans text-slate-200">
      ...
      {/* Header */}
      <header className="relative z-10 w-full p-6 sm:p-10 flex flex-col sm:flex-row items-center justify-between gap-4">
        <motion.div 
        ...
        >
          ...
          <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center overflow-hidden border-2 border-slate-800 shadow-[0_0_15px_rgba(220,38,38,0.3)] shrink-0">
             <img src="/logo.jpeg" alt="APEC Logo" className="w-full h-full object-contain p-1" onError={(e) => {
               ...
               (e.target as HTMLImageElement).onerror = (e) => {
                 (e.target as HTMLImageElement).src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="%23dc2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>';
                 (e.target as HTMLImageElement).className = 'w-8 h-8 object-contain';
                 (e.target as HTMLImageElement).onerror = null;
               };
             }} />
          </div>
          <div className="flex flex-col text-center sm:text-left">
            <span className="font-bold text-2xl text-white tracking-widest uppercase leading-tight">APEC</span>
            <span className="text-[10px] text-red-500 font-bold tracking-[0.2em] uppercase leading-none">Power Solutions Pvt. Ltd</span>
          </div>
        </motion.div>
      ...
      {/* Main Content Area */}
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center p-6 lg:p-12 text-center mt-[-40px]">
        <div className="max-w-4xl w-full flex flex-col items-center gap-12">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-800/50 border border-slate-700/50 w-fit mx-auto">
               <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
              </span>
              <span className="text-xs font-medium text-slate-300 tracking-wider uppercase">System Alpha Build</span>
            </div>
            ...
            <div className="pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-2xl mx-auto text-left">
            ...
            <div className="pt-8 flex flex-col items-center gap-4">
              <button 
                disabled
                className="px-8 py-3.5 rounded-lg bg-slate-800 text-slate-500 font-medium text-sm cursor-not-allowed border border-slate-700 w-fit flex items-center justify-center gap-3 shadow-lg"
              >
                 <Settings className="w-4 h-4 animate-spin" style={{ animationDuration: '3s' }} /> 
                 Admin Portal (Locked)
              </button>

              <div className="flex items-center gap-2 bg-slate-900/80 px-4 py-2 rounded-full border border-slate-800 shadow-sm mt-4">
          ...
          >
            <div className="relative w-full max-w-[280px] aspect-square flex items-center justify-center">
              ...
              <motion.div 
                ...
                className="absolute inset-2 rounded-full border border-slate-700/50 border-t-red-500/30 border-r-blue-500/30"
              />
              <motion.div 
                animate={{ rotate: -360 }}
                transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
                className="absolute inset-8 rounded-full border border-dashed border-slate-700/80"
              />
              
              {/* Center module */}
              <div className="relative z-10 w-44 h-44 rounded-full bg-slate-900 border border-slate-800 shadow-[0_0_40px_rgba(220,38,38,0.15)] flex flex-col items-center justify-center p-6">
                 
                 <motion.div
                    animate={{ y: [0, -6, 0] }}
                    transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                 >
                   <Construction className="w-14 h-14 text-red-500 mb-2" />
                 </motion.div>
                 
                 <div className="text-center">
                   <div className="text-2xl font-mono font-bold text-white tracking-widest">72<span className="text-red-500 text-lg">%</span></div>
                 </div>

                 {/* Minimal progress bar */}
                 <div className="absolute bottom-8 w-20 h-1 bg-slate-800 rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: "0%" }}
                      animate={{ width: "72%" }}
              ...
              <motion.div 
                 ...
                 className="absolute top-4 right-4 w-12 h-12 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center shadow-lg"
              >
              ...
              <motion.div 
                 ...
                 className="absolute bottom-6 left-2 w-14 h-14 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center shadow-lg"
              >
      ...
      {/* Footer */}
      <footer className="relative z-10 p-6 sm:p-10 text-center flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-slate-800/50">
