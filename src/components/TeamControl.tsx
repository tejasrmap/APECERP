...
export default function TeamControl() {
  ...
  const [controlTab, setControlTab] = useState<'registry' | 'roles'>('registry');
  ...
  const [editAccessRole, setEditAccessRole] = useState('User');
  ...
  const handleTogglePriority = async (id: string, currentRole: string, userName: string) => {
    ...
    // Cycle roles: User -> Admin -> Custom Roles in rolesList -> User
    const roleOrder = ['User', 'Admin', ...rolesList.map((r: any) => r.name)];
    const curIndex = roleOrder.findIndex(r => r.toLowerCase() === currentRole.toLowerCase());
    const nextIndex = curIndex === -1 ? 0 : (curIndex + 1) % roleOrder.length;
    ...
    try {
      ...
      await addDoc(collection(db, 'activities'), {
        ...
        desc: `"${userName}" was assigned to the "${nextRole}" role`,
        type: 'settings',
        timestamp: Timestamp.now()
      });
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {!isAddingUser && !isEditingProfile && !isAddingRole && (
          <div className="flex items-center gap-3">
            {/* View Tab Toggle */}
            <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-900">
              <button
                type="button"
                onClick={() => setControlTab('registry')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  controlTab === 'registry' ? 'bg-cyan-500 text-slate-950 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
                Team Registry
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  controlTab === 'roles' ? 'bg-cyan-500 text-slate-950 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
                Roles & Permissions
              </button>
            ...
            {controlTab === 'registry' ? (
              <button 
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-cyan-400 hover:from-cyan-400 hover:to-cyan-300 text-slate-950 text-xs font-bold flex items-center gap-1.5 transition-all shadow-[0_4px_12px_rgba(6,182,212,0.15)] hover:shadow-lg disabled:opacity-50 cursor-pointer"
              >
        ...
        {(isAddingUser || isEditingProfile || isAddingRole) && (
          ...
          >
            ...
            {isAddingUser ? 'Back to Registry' : isAddingRole ? 'Back to Roles' : 'Back to Profile'}
          </button>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        {isAddingUser ? (
                      ...
                      {openDropdown === 'priority' && (
                        ...
                        >
                          {[
                            ...
                            ...rolesList.map((r: any) => ({ value: r.name, label: `${r.name} (Custom)` }))
                          ].map((role) => (
                            <button
                              key={role.value}
                              type="button"
              ...
              <div className="space-y-3 p-4 bg-slate-950/20 rounded-xl border border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1 relative">
                    ...
                    <button
                      type="button"
                      onClick={() => setOpenEditDropdown(openEditDropdown === 'priority' ? null : 'priority')}
                      className="w-full bg-slate-950/60 border border-slate-800 text-slate-100 rounded-xl py-2.5 px-3.5 focus:outline-none focus:border-cyan-500/50 text-xs flex justify-between items-center transition-all shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] focus:ring-1 focus:ring-cyan-500/10 cursor-pointer text-left"
                    >
                      <span>
                        {editAccessRole === 'Admin' 
                          ...
                          : editAccessRole === 'User' 
                            ? 'User (Standard)' 
                            : `${editAccessRole} (Custom)`}
                      </span>
                      <ChevronDown className={`w-3.5 h-3.5 text-slate-500 transition-transform duration-200 ${openEditDropdown === 'priority' ? 'rotate-180' : ''}`} />
                    </button>
                    <AnimatePresence>
                      {openEditDropdown === 'priority' && (
                        <motion.div
                          initial={{ opacity: 0, y: 5 }}
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 5 }}
                        ...
                        >
                          {[
                            ...
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                setEditAccessRole(role.value);
                                setOpenEditDropdown(null);
                              }}
                              className={`w-full text-left px-3 py-2 text-xs rounded-lg transition-colors flex items-center justify-between ${
                                editAccessRole === role.value 
                                  ? 'bg-cyan-500/10 text-cyan-400 font-semibold' 
                                  : 'text-slate-300 hover:bg-slate-800/40 hover:text-slate-100'
                              }`}
                            >
                              <span>{role.label}</span>
                              {editAccessRole === role.value && <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />}
                            </button>
                          ))}
                        </motion.div>
                      )}
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="w-full max-w-2xl glass-card p-6 rounded-[2rem] relative overflow-hidden shadow-[0_12px_40px_rgba(0,0,0,0.3)] mx-auto"
          >
            <h4 className="text-sm font-bold text-slate-100 mb-4 flex items-center gap-2">
                  ...
                  <div key={role.id} className="relative p-6 rounded-2xl glass-card border border-white/5 flex flex-col justify-between hover:border-cyan-500/20 transition-all duration-300">
                    {!role.isSystem && (
                      <button
                        ...
                        disabled={isDbActionLoading}
                        className="absolute top-4 right-4 p-1.5 text-slate-500 hover:text-rose-500 transition-colors rounded hover:bg-rose-955/20 disabled:opacity-50"
                            ...
                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                              isMemberAdmin 
                                ...
                                : m.accessRole && m.accessRole !== 'User'
                                  ? 'bg-indigo-955/40 text-indigo-400 border border-indigo-500/25 shadow-[0_0_10px_rgba(99,102,241,0.15)]'
                ...
                {!isEditingProfile && (
                  <button 
                    onClick={() => {
                      ...
                      setEditAccessRole(selectedProfile.accessRole || 'User');
